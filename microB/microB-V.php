...
if (!isset($_POST['submit'])) {
    ...
    $data = array();
    for ($i=6; $i<=18; $i+=1) {
        $voltageB = mysqli_query($connect, "SELECT AVG(voltageB) as avg_voltage FROM pzem_data WHERE DATE(created_at) = '$tgl_real' AND TIME(created_at) BETWEEN ADDTIME('$i:00:00', '-1:00:00') AND '$i:00:00'");
        $data_voltageB = mysqli_fetch_array($voltageB);
        $data[] = round($data_voltageB['avg_voltage'], 1);
    }
    ...
    $data_v = array(
        ...
        'datasets' => array(
            array(
                'label' => 'Tegangan',
                'fill' => true,
                'backgroundColor' => 'rgb(253, 233, 5, .5)',
                'borderColor' => 'rgb(253, 233, 5, 2)',
                'data' => $data
            )
        )
    );

    $voltageB = mysqli_query($connect, "SELECT voltageB FROM pzem_data WHERE DATE(created_at) = '$tgl_real' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$wkt_awl' AND '$wkt_akr'");
    // -----------------------------------------------------------------------------------
    // Higher Voltage
    $result1 = mysqli_query($connect, "SELECT MAX(voltageB) as max_teg FROM pzem_data WHERE DATE(created_at) = '$tgl_real'");
    $higher = mysqli_fetch_array($result1);
    $max_teg = $higher['max_teg'];
    $max_teg_formatted = number_format($max_teg, 1);

    // Average Voltage
    $result2 = mysqli_query($connect, "SELECT AVG(voltageB) as avg_teg FROM pzem_data WHERE DATE(created_at) = '$tgl_real'");
    ...
    $query_data = mysqli_query($connect, "SELECT voltageB FROM pzem_data WHERE DATE(created_at) = '$tgl_real' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$wkt_awl' AND '$wkt_akr'");
    ...
    $data = array();
    for ($i=6; $i<=18; $i+=1) {
        $voltageB = mysqli_query($connect, "SELECT AVG(voltageB) as avg_voltage FROM pzem_data WHERE DATE(created_at) = '$tgl_awal' AND TIME(created_at) BETWEEN ADDTIME('$i:00:00', '-1:00:00') AND '$i:00:00'");
        $data_voltageB = mysqli_fetch_array($voltageB);
        $data[] = round($data_voltageB['avg_voltage'], 1);
    }
    ...
    // CONDITION
    $voltageB = mysqli_query($connect, "SELECT voltageB FROM pzem_data WHERE DATE(created_at) = '$tgl_awal' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$wkt_awl' AND '$wkt_akr'");
    // -----------------------------------------------------------------------------------
    // Higher Voltage
    $result1 = mysqli_query($connect, "SELECT MAX(voltageB) as max_teg FROM pzem_data WHERE DATE(created_at) = '$tgl_awal'");
    $higher = mysqli_fetch_array($result1);
    $max_teg = $higher['max_teg'];
    $max_teg_formatted = number_format($max_teg, 1);

    // Average Voltage
    $result2 = mysqli_query($connect, "SELECT AVG(voltageB) as avg_teg FROM pzem_data WHERE DATE(created_at) = '$tgl_awal'");
    $avg = mysqli_fetch_array($result2);
    $avg_teg = $avg["avg_teg"];
    $avg_teg_formatted = number_format($avg_teg, 1);
    // ---------
    ...
    // syntax for the scroll
    $query_data = mysqli_query($connect, "SELECT voltageB FROM pzem_data WHERE DATE(created_at) = '$tgl_awal' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$wkt_awl' AND '$wkt_akr' AND HOUR(created_at) % 2 = 0 ORDER BY ID ASC");
            ...
            <div class="navbar">
                ...
                <ul class="navigation">
                    ...
                    <li>
                        ...
                        <ul class="dropdown2">
                            <li><a href="microB-V.php" id="active"><i class="fa-regular fa-v"></i>
                                    <label class="text-dp">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Tegangan</label></a></li>
                    ...
                    <div class="grafikP">
                        <div class="grafik">
                            ...
                            <div style="text-align: center;height: 30px; align-items:center; font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
                                STATISTIK RATA-RATA PERJAM TEGANGAN MICROINVENTER B
                            </div>
