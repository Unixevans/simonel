...
if (!isset($_POST['submit'])) {
    ...
    $data = array();
    for ($i=6; $i<=18; $i+=1) {
        $dayanyataB = mysqli_query($connect, "SELECT AVG(dayanyataB) as avg_dayanyata FROM pzem_data WHERE DATE(created_at) = '$tgl_real' AND TIME(created_at) BETWEEN ADDTIME('$i:00:00', '-1:00:00') AND '$i:00:00'");
        $data_dayanyataB = mysqli_fetch_array($dayanyataB);
        $data[] = round($data_dayanyataB['avg_dayanyata'], 1);
    }
    ...
    $data_v = array(
        ...
        'datasets' => array(
            array(
                ...
                'borderColor' => "rgba(255, 92, 92, 2)",
                'data' => $data
            )
        )
    );      

    $dayanyataB = mysqli_query($connect, "SELECT dayanyataB FROM pzem_data WHERE DATE(created_at) = '$tgl_real' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$wkt_awl' AND '$wkt_akr' AND HOUR(created_at) % 2 = 0 ORDER BY ID ASC");
    // -----------------------------------------------------------------------------------
    // Higher Voltage
    $result1 = mysqli_query($connect, "SELECT MAX(dayanyataB) as max_teg FROM pzem_data WHERE DATE(created_at) = '$tgl_real'");
    ...
    // Average Voltage
    $result2 = mysqli_query($connect, "SELECT AVG(dayanyataB) as avg_teg FROM pzem_data WHERE DATE(created_at) = '$tgl_real'");
    ...
    $query_data = mysqli_query($connect, "SELECT dayanyataB FROM pzem_data WHERE DATE(created_at) = '$tgl_real' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$wkt_awl' AND '$wkt_akr' AND HOUR(created_at) % 2 = 0 ORDER BY ID ASC");
    ...
    $data = array();
    for ($i=6; $i<=18; $i+=1) {
        $dayanyataB = mysqli_query($connect, "SELECT AVG(dayanyataB) as avg_dayanyata FROM pzem_data WHERE DATE(created_at) = '$tgl_awal' AND TIME(created_at) BETWEEN ADDTIME('$i:00:00', '-1:00:00') AND '$i:00:00'");
        $data_dayanyataB = mysqli_fetch_array($dayanyataB);
        $data[] = round($data_dayanyataB['avg_dayanyata'], 1);
    }
    ...
    // CONDITION
    $dayanyataB = mysqli_query($connect, "SELECT dayanyataB FROM pzem_data WHERE DATE(created_at) = '$tgl_awal' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$wkt_awl' AND '$wkt_akr' AND HOUR(created_at) % 2 = 0 ORDER BY ID ASC");
    // -----------------------------------------------------------------------------------
    // Higher Voltage
    $result1 = mysqli_query($connect, "SELECT MAX(dayanyataB) as max_teg FROM pzem_data WHERE DATE(created_at) = '$tgl_awal'");
    $higher = mysqli_fetch_array($result1);
    $max_teg = $higher['max_teg'];
    $max_teg_formatted = number_format($max_teg, 1);
    ...
    // Average Voltage
    $result2 = mysqli_query($connect, "SELECT AVG(dayanyataB) as avg_teg FROM pzem_data WHERE DATE(created_at) = '$tgl_awal'");
    $avg = mysqli_fetch_array($result2);
    $avg_teg = $avg["avg_teg"];
    $avg_teg_formatted = number_format($avg_teg, 1);
    // ---------
    ...
    // syntax for the scroll
    $query_data = mysqli_query($connect, "SELECT dayanyataB FROM pzem_data WHERE DATE(created_at) = '$tgl_awal' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$wkt_awl' AND '$wkt_akr' AND HOUR(created_at) % 2 = 0 ORDER BY ID ASC");
                    ...
                    <li>
                        ...
                        <ul class="dropdown2">
                            ...
                            <li><a href="microB-VA.php"><i class="fa-regular fa-v"></i><i class="fa-regular fa-a"></i>
                                    <label class="text-dp">&nbsp;&nbsp;&nbsp;&nbsp;Daya Semu</label></a></li>
                            <li><a href="microB-W.php" id="active"><i class="fa-regular fa-w"></i>
                                    <label class="text-dp">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Daya Nyata</label></a></li>
                    ...
                    <div class="grafikP">
                        <div class="grafik">
                            ...
                            <div style="text-align: center;height: 30px; align-items:center; font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
                                STATISTIK DAYA NYATA MICROINVENTER B
                            </div>
