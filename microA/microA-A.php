...
if (!isset($_POST['submit'])) {
    ...
    $data = array();
    for ($i=6; $i<=18; $i+=1) {
        $currentA = mysqli_query($connect, "SELECT AVG(currentA) as avg_current FROM pzem_data WHERE DATE(created_at) = '$tgl_real' AND TIME(created_at) BETWEEN ADDTIME('$i:00:00', '-1:00:00') AND '$i:00:00'");
        $data_currentA = mysqli_fetch_array($currentA);
        $data[] = round($data_currentA['avg_current'], 1);
    }
    ...
    $currentA = mysqli_query($connect, "SELECT currentA FROM pzem_data WHERE DATE(created_at) = '$tgl_real' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$wkt_awl' AND '$wkt_akr'");
    // -----------------------------------------------------------------------------------
    // Higher Voltage
    $result1 = mysqli_query($connect, "SELECT MAX(currentA) as max_teg FROM pzem_data WHERE DATE(created_at) = '$tgl_real'");
    $higher = mysqli_fetch_array($result1);
    $max_teg = $higher['max_teg'];
    $max_teg_formatted = number_format($max_teg, 1);

    // Average Voltage
    $result2 = mysqli_query($connect, "SELECT AVG(currentA) as avg_teg FROM pzem_data WHERE DATE(created_at) = '$tgl_real'");
    ...
    $query_data = mysqli_query($connect, "SELECT currentA FROM pzem_data WHERE DATE(created_at) = '$tgl_real' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$wkt_awl' AND '$wkt_akr' AND MINUTE(created_at) % 2 = 0 ORDER BY ID ASC");
    ...
    $data = array();
    for ($i=6; $i<=18; $i+=1) {
        $currentA = mysqli_query($connect, "SELECT AVG(currentA) as avg_current FROM pzem_data WHERE DATE(created_at) = '$tgl_awal' AND TIME(created_at) BETWEEN ADDTIME('$i:00:00', '-1:00:00') AND '$i:00:00'");
        $data_currentA = mysqli_fetch_array($currentA);
        $data[] = round($data_currentA['avg_current'], 1);
    }
    ...
    $currentA = mysqli_query($connect, "SELECT currentA FROM pzem_data WHERE DATE(created_at) = '$tgl_awal' AND DATE_FORMAT(created_at, '%H:%i:s') BETWEEN '$wkt_awl' AND '$wkt_akr'");
    // -----------------------------------------------------------------------------------
    // Higher Voltage
    $result1 = mysqli_query($connect, "SELECT MAX(currentA) as max_teg FROM pzem_data WHERE DATE(created_at) = '$tgl_awal'");
    $higher = mysqli_fetch_array($result1);
    $max_teg = $higher['max_teg'];
    $max_teg_formatted = number_format($max_teg, 1);

    // Average Voltage
    $result2 = mysqli_query($connect, "SELECT AVG(currentA) as avg_teg FROM pzem_data WHERE DATE(created_at) = '$tgl_awal'");
    $avg = mysqli_fetch_array($result2);
    $avg_teg = $avg["avg_teg"];
    $avg_teg_formatted = number_format($avg_teg, 1);
    // ---------
    ...
    // syntax for the scroll
    $query_data = mysqli_query($connect, "SELECT currentA FROM pzem_data WHERE DATE(created_at) = '$tgl_awal' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$wkt_awl' AND '$wkt_akr' AND HOUR(created_at) % 2 = 0 ORDER BY ID ASC");
                ...
                <ul class="navigation">
                    ...
                    <li>
                        ...
                        <ul class="dropdown1">
                            ...
                            <li><a href="microA-A.php" id="active"><i class="fa-regular fa-a"></i>
                                    <label class="text-dp">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Arus</label></a></li>
                            <li><a href="microA-VA.php"><i class="fa-regular fa-v"></i><i class="fa-regular fa-a"></i>
                        ...
                        <div class="scrool">
                            <div class="in">
                                ...
                                <div class="text_ttl">
                                    ...
                                    <div class="ttl_2">Berikut Statistik Arus Microinventer A</div>
                                </div>
                            </div>
                            <div class="scrollbar">
                                <?php foreach($data as $row => $value) : ?>
                                    <div class="item">
                                        <div class="item-content">
                                            <span class="scr-1"><?php echo $value; ?> A</span>
                                            <span><i class="fa-solid fa-bolt"></i></span>
                                            <span class="scr-2"><?php echo $labels[$row]; ?></span>
                    ...
                    <div class="grafikP">
                        <div class="grafik">
                            ...
                            <div style="text-align: center;height: 30px; align-items:center; font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
                                STATISTIK RATA-RATA PERJAM ARUS MICROINVENTER A
                            </div>
