...
// FILTER FOR VIEWING THE DATA Y AND TIME X
$wkt_awl = '06:00:00';
$wkt_akr = '18:00:00';

// FILTER FOR VIEWING THE SCROLL DATA
$tm_str = '06:00:00';
$tm_end = '18:00:00';

// Syntax Query for Data Sumbu X
// THIS LOGIC FOR THE FILTER LIMIT DATA GRAPHIC
$hours = array('06', '08', '10', '12', '14', '16', '18');
...
if (!isset($_POST['submit'])) {
    ...
    for ($i=6; $i<=18; $i+=1) {
        ...
        $data_voltageA = mysqli_fetch_array($voltageA);
        $data[] = round($data_voltageA['avg_voltage'], 1);
    }
    ...
    $voltageA = mysqli_query($connect, "SELECT voltageA FROM pzem_data WHERE DATE(created_at) = '$tgl_real' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$wkt_awl' AND '$wkt_akr'");
    // Query for scroll
    $query_data = mysqli_query($connect, "SELECT voltageA FROM pzem_data WHERE DATE(created_at) = '$tgl_real' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$tm_str' AND '$tm_end' AND HOUR(created_at) % 2 = 0");

    $result1 = mysqli_query($connect, "SELECT MAX(voltageA) as max_teg FROM pzem_data WHERE DATE(created_at) = '$tgl_real'");
    $higher = mysqli_fetch_array($result1);
    $max_teg = $higher['max_teg'];
    $max_teg_formatted = number_format($max_teg);

    // Average Voltage
    $result2 = mysqli_query($connect, "SELECT AVG(voltageA) as avg_teg FROM pzem_data WHERE DATE(created_at) = '$tgl_real'");
    ...
    $data = array();
    for ($i=6; $i<=18; $i+=1) {
        $voltageA = mysqli_query($connect, "SELECT AVG(voltageA) as avg_voltage FROM pzem_data WHERE DATE(created_at) = '$tgl_awal' AND TIME(created_at) BETWEEN ADDTIME('$i:00:00', '-1:00:00') AND '$i:00:00'");
        $data_voltageA = mysqli_fetch_array($voltageA);
        $data[] = round($data_voltageA['avg_voltage'], 1);
    }
    ...
    // CONDITION
    $voltageA = mysqli_query($connect, "SELECT voltageA FROM pzem_data WHERE DATE(created_at) = '$tgl_awal' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$wkt_awl' AND '$wkt_akr'");
    // -----------------------------------------------------------------------------------
    // Higher Voltage
    $result1 = mysqli_query($connect, "SELECT MAX(voltageA) as max_teg FROM pzem_data WHERE DATE(created_at) = '$tgl_awal'");
    $higher = mysqli_fetch_array($result1);
    $max_teg = $higher['max_teg'];
    $max_teg_formatted = number_format($max_teg);

    // Average Voltage
    $result2 = mysqli_query($connect, "SELECT AVG(voltageA) as avg_teg FROM pzem_data WHERE DATE(created_at) = '$tgl_awal'");
    $avg = mysqli_fetch_array($result2);
    $avg_teg = $avg["avg_teg"];
    $avg_teg_formatted = number_format($avg_teg, 1);
    // ---------
    // syntax for the scroll
    $query_data = mysqli_query($connect, "SELECT voltageA FROM pzem_data WHERE DATE(created_at) = '$tgl_awal' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$wkt_awl' AND '$wkt_akr' AND HOUR(created_at) % 2 = 0 ORDER BY ID ASC");
...
<body>
    <div class="Container">
        <div class="wrapper">
            ...
            <div class="h1">
                <h2 class="vt-h2">TEGANGAN</h2>
            </div>
        </div>
        <div class="bungkusan">
            <div class="navbar">
                <div class="monitor" id="slide-hilang">
                    <h4 class="mntrng">MONITORING</h4>
                </div>
                <ul class="navigation">
                    <li><a href="../index.php">
                    ...
                    <li>
                        ...
                        <ul class="dropdown1">
                            <li><a href="microA-V.php" id="active"><i class="fa-regular fa-v"></i>
                                ...
                                <?php foreach($data as $row => $value) : ?>
                                    <div class="item">
                                        <div class="item-content">
                                            <span class="scr-1"><?php echo $value; ?> V</span>
                    ...
                    <div class="grafikP">
                        <div class="grafik">
                            ...
                            <div style="text-align: center;height: 30px; align-items:center; font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
                                STATISTIK RATA-RATA PERJAM TEGANGAN MICROINVENTER A
                            </div>
