...
if (!isset($_POST['submit'])) {
    ...
    $data = array();
    for ($i=6; $i<=18; $i+=1) {
        $dayanyataA = mysqli_query($connect, "SELECT AVG(dayanyataA) as avg_dayanyata FROM pzem_data WHERE DATE(created_at) = '$tgl_real' AND TIME(created_at) BETWEEN ADDTIME('$i:00:00', '-1:00:00') AND '$i:00:00'");
        $data_dayanyataA = mysqli_fetch_array($dayanyataA);
        $data[] = round($data_dayanyataA['avg_dayanyata'], 1);
    }
    ...
    $dayanyataB = mysqli_query($connect, "SELECT dayanyataB FROM pzem_data WHERE DATE(created_at) = '$tgl_real' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$wkt_awl' AND '$wkt_akr'");
    // -----------------------------------------------------------------------------------
    // Higher Voltage
    ...
    $data = array();
    for ($i=6; $i<=18; $i+=1) {
        $dayanyataA = mysqli_query($connect, "SELECT AVG(dayanyataA) as avg_dayanyata FROM pzem_data WHERE DATE(created_at) = '$tgl_awal' AND TIME(created_at) BETWEEN ADDTIME('$i:00:00', '-1:00:00') AND '$i:00:00'");
        $data_dayanyataA = mysqli_fetch_array($dayanyataA);
        $data[] = round($data_dayanyataA['avg_dayanyata'], 1);
    }
    ...
    // CONDITION
    $dayanyataB = mysqli_query($connect, "SELECT dayanyataB FROM pzem_data WHERE DATE(created_at) = '$tgl_awal' AND DATE_FORMAT(created_at, '%H:%i:%s') BETWEEN '$wkt_awl' AND '$wkt_akr'");
    // -----------------------------------------------------------------------------------
    // Higher Voltage
                ...
                <ul class="navigation">
                    ...
                    <li>
                        ...
                        <ul class="dropdown1">
                            ...
                            <li><a href="microA-W.php" id="active"><i class="fa-regular fa-w"></i>
                    ...
                    <div class="grafikP">
                        <div class="grafik">
                            ...
                            <div style="text-align: center;height: 30px; align-items:center; font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
                                STATISTIK RATA-RATA PERJAM DAYA NYATA MICROINVENTER A
                            </div>
